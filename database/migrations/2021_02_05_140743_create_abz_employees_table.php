     ...
     public function up()
    {
	  if (!Schema::hasTable('abz_employees')) { //my fix for migration
        Schema::create('abz_employees', function (Blueprint $table) {
			...
			$table->unsignedInteger('rank_id')->nullable()->comment = 'Rank Id from table {abz_ranks} (ForeignKey)'; 
			...
			$table->unsignedInteger('superior_id')->nullable()->comment = 'Superior Id from this same table itself {abz_employees} (ForeignKey)'; 
